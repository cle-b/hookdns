...
def test_real_getaddrinfo_with_name_ipv4():
    r = socket.getaddrinfo("localhost", 80, family=socket.AF_INET, proto=socket.IPPROTO_TCP)
...
def test_real_getaddrinfo_with_ipv4():
    r = socket.getaddrinfo("127.0.0.1", 80, family=socket.AF_INET, proto=socket.IPPROTO_TCP)
    for (_, _, _, _, sockaddr) in r:
        assert sockaddr == ("127.0.0.1", 80)
...
def test_real_getaddrinfo_with_public_fqdn_ipv4():
    r = socket.getaddrinfo("example.org", 80, family=socket.AF_INET, proto=socket.IPPROTO_TCP)
    for (_, _, _, _, (ipaddr, _)) in r:
        assert ipaddr != "127.0.0.1"

def test_real_getaddrinfo_with_public_fqdn_ipv6():
    r = socket.getaddrinfo("example.org", 80, family=socket.AF_INET6, proto=socket.IPPROTO_TCP)
...
def test_patch_decorator_with_unknown_hostname():
    assert socket.getaddrinfo("UNKNOWN_HOST_AZ", 80) == socket.getaddrinfo("127.0.0.1", 80)
