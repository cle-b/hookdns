...
def test_patch_decorator_with_unknown_hostname(httpd, httpdport):
    assert "from unknown_host_az" in requests.get("http://unknown_host_az:%d/" % httpdport).text
...
def test_patch_contextmanager_with_unknown_hostname(httpd, httpdport):
    with hosts({"unknown_host_az": "127.0.0.1"}): # don't work if hostname is in upper case
        assert "from unknown_host_az" in requests.get("http://unknown_host_az:%d/" % httpdport).text
